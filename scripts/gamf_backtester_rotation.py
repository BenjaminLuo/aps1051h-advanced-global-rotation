...
def backtest():
    # Using Unified Engine in BOND_ROTATION mode
    return unified_backtest(hedge_mode=HedgeMode.BOND_ROTATION)

def plot_results(s_rets, b_rets, w_df, r_df, label="V13-ROTATION"):
    plt.close('all')
    fig, axes = plt.subplots(2, 1, figsize=(14, 12), gridspec_kw={'height_ratios': [3, 1]})
    cum_s = (1 + s_rets).cumprod()
    cum_b = (1 + b_rets).cumprod()
    axes[0].plot(cum_s, label=f"GAMF {label}", color='#3498DB', linewidth=2.5)
    axes[0].plot(cum_b, label="SPY (Benchmark)", color='#34495E', alpha=0.6)
    axes[0].set_title(f"GAMF ROTATION: {label} (Long-Term Horizon)", fontsize=16, fontweight='bold')
    ...
    plt.savefig('figures/presentation/v13_rotation_dashboard.png', dpi=300)
